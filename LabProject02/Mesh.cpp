...
XMFLOAT3 CVertex::Getxmf3Position() const
{
...
int CPolygon::GetnVertices() const
{
	return m_nVertices;
}

CVertex* CPolygon::GetpVertices()
{
	return m_pVertices;
}
...
void CMesh::Render(HDC hDCFrameBuffer)
{
	...
	for (int j = 0; j < m_nPolygons; j++)
	{
		...
		{
			// 다음 벡터 가져오기
			XMFLOAT3 f3CurrentProject = CGraphicsPipeline::Project(pVertices[i].Getxmf3Position());
			...
			{
				// z값도 검사후 현재 벡터와 이전 백터 둘 중 하나라도 들어왔다면 선을 그린다
